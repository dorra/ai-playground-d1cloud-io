...
Redactor.add('plugin', 'emoji', {
    ...
    _load() {
        this._createPanel();
        let sections = this._buildEmoji(this.$panel, this.handleStr);
        if (sections === 0) {
            this._hide();
        }
    },
    _createPanel() {
        ...
        if (this.$panel.length === 0) {
            this.$panel = this.dom('<div>').addClass('rx-panel rx-panel-emoji').css('max-width', '372px');
        ...
        this._buildPanelPosition();
    },
    _buildEmoji($modal, filter) {
        let items = this.opts.get('emoji.items');
        let sections = 0;

        for (let [name, section] of Object.entries(items)) {
            let $section = this.dom('<div class="rx-emoji-section">');
            let $title = this.dom('<div class="rx-emoji-title">');
            let $box = this.dom('<div class="rx-emoji-box">');
            let size = 0;

            $title.html(name.charAt(0).toUpperCase() + name.slice(1));
            ...
            for (let [key, value] of Object.entries(section)) {
                ...
                let $item = this.dom('<span class="rx-emoji-item">');
                $item.html(value);
